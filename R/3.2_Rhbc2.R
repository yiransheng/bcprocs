Rhbc2(k, n1, c1, n2, c2) %when% {
  ...
  structure(list(
    ...
    M=0L, # M'th step
    nmax=n1, # maximum allowed number of observations to take from each treatment for stage 1
...
stage.one.Rhbc2(x, vec) %as% {
  warning("Incorrect stage or corrupted Rhbc2 object.")
  x
}

stage.two.Rhbc2(proc, x12, xk2) %when% {
  proc$stage == 2L
  proc$decision != 0L
  !any(is.na(x12))
  !any(is.na(xk2))
...
stage.two.Rhbc2(proc, ...)  %as% {
  stop("Invalid data.")
}

run.experiment.Rhbc2 <- function(x, data) {
...
summary.Rhbc2 <- function(x) {
  cat('Procedure Rhbc2, see Buzaianu and Chen (2009)\n')
  ...
  if (isStageIComplete) {
    ...
    cat('Used', sum(x$nobs), 'observations in total in this stage\n')
  } else {
  ...
  if (x$stage == 2L) {
    ...
    cat('Stage II complted, choose:')
    if (x$decision == 0L) {
      cat('Control\n')
    } else {
      cat('Treatement', x$decision, '\n')
    }
    cat('Used', sum(x$nobs2), 'observations in total in this stage\n')
  }
